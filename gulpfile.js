...
gulp.task('minifyCss', function () {
  return gulp.src('./www/css/**/*.css')      //压缩的文件
    .pipe(minifyCss())    //执行压缩
    .pipe(gulp.dest('./yuanhengxiang/css/'));  //输出文件夹
});
...
gulp.task('minifyJs', function () {
  return gulp.src('./www/js/**/*.js/')
    .pipe(uglify())    //压缩
    .pipe(gulp.dest('./yuanhengxiang/js/'));  //输出
});
...
gulp.task('minImages', function(){
  return gulp.src('./www/img/**/*.+(png|jpg|gif|svg)')
    ...
    .pipe(gulp.dest('./yuanhengxiang/img/'));
});

/*复制html*/
gulp.task('copyHtml', function () {
  return gulp.src('./www/templates/**/*.html')      //复制的文件
    .pipe(gulp.dest('./yuanhengxiang/templates/'));  //输出文件夹
});

/*复制Lib*/
gulp.task('copyLib', function () {
  return gulp.src('./www/lib/**')      //复制的文件
    .pipe(gulp.dest('./yuanhengxiang/lib/'));  //输出文件夹
});
/*执行压缩前，先删除文件夹里的内容*/
gulp.task('clean', function (cb) {
  del(['./yuanhengxiang/css', './yuanhengxiang/js'], cb);
});
